...
class InvoiceController extends Controller
{
    ...
    {
        //aqui estou falando que os meus métodos store e update precisarão de autenticação e que para liberar o acesso o token tem que ter ou o invoice-store ou o user-update para liberar o acesso 
    ...
    public function store(Request $request)
{
    ...
    $created = Invoice::create($validator->validated());  //este validated são todos os dados validados
    ...
    {
        return new InvoiceResource($invoice);       //para mostrar no estilo bind
    }
    ...
    public function update(Request $request, Invoice $invoice)
    {
        $validator = Validator::make($request->all(), [
            'user_id' => 'required',
            'type' => 'required|max:1|in:' . implode(',', ['b', 'c', 'a']),
            'paid' => 'required|numeric|between:0, 1',
        ...
        if($updated){
            return $this->response('invoice updated', 200, new InvoiceResource($invoice->load('user')));        //modo baind
        }
